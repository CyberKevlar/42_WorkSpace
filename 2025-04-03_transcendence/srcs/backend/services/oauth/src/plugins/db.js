...
async function dbPlugin(fastify, options) {
  ...
  fastify.decorate("userDB", {
    ...
    async createUser(username, password, oauth = 0) {
      try {
        if (!username || !password) {
          throw new Error('Username and password are required');  // Verifica que username y password no estén vacíos
        }
        const hashedPassword = await bcrypt.hash(password, 10);  // Encripta la contraseña
        const stmt = db.prepare('INSERT INTO users (username, password, oauth) VALUES (?, ?, ?)');  // Prepara statement de inserción
        const result = stmt.run(username, hashedPassword, oauth);   // Ejecuta la inserción
        return { 
